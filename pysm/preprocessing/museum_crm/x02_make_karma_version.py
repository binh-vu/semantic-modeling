...
model_json_dir = Path(config.datasets[dataset].as_path()) / "karma-version" / "models-json"
...
for tbl in get_data_tables(dataset):
    ...
    serializeJSON(sm.to_karma_json_model(ont), model_json_dir / f"{tbl.id}-model.json", indent=4)