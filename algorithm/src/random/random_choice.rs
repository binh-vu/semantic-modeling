...
// This code live here:
// https://raw.githubusercontent.com/StefanoD/Rust_Random_Choice/master/src/lib.rs
...
//! extern crate rand;
//! use algorithm::random::random_choice::random_choice;
//!
//! # fn main() {
...
//!     let weights: Vec<f64> = vec![5.6, 7.8, 9.7, 1.1, 2.0];
//!
//!     let rng = rand::StdRng::from_seed([120; 32]);
//!
...
pub struct RandomChoice<RNG: RngCore> {
    rng: RNG,
}
...
impl<RNG: RngCore> RandomChoice<RNG> {
    ...
    pub fn random_choice_f64<'a, T>(&mut self,
        ...
        // next_f64() ∈ [0.0, 1.0)
        let spin = self.rng.gen_range(0.0, 1.0) * spoke_gap;
    ...
    pub fn random_choice_f64_without_replace(&mut self, weights: &[f64], n: usize) -> Vec<usize> {
        if weights.len() == 0 || n == 0 {
            return Vec::new();
        }

        let mut cdf = CDF::new(&weights);

        (0..n).map(|i| {
                let prob = self.rng.gen_range(0.0, 1.0);
                cdf.remove_sample(&prob)
            })
            .collect::<Vec<_>>()
    }
}